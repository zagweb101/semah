...
export function MoodboardManager({ projectId, moodboardId, items, mockMode }: { projectId: string; moodboardId: string | null; items: Item[]; mockMode: boolean }) {
  ...
  function handleCreate() { startTransition(async () => { const result: any = await createMoodboardAction(projectId); if (result?.error) toast.error(result.error); else { toast.success("تم إنشاء Mood Board"); router.refresh(); } }); }
  function handleGenerate() { if (!prompt.trim()) { toast.error("اكتب وصفًا"); return; } setError(null); startTransition(async () => { const result: any = await generateMoodboardImagesAction(projectId, [prompt.trim()]); if (result?.error) { setError(result.error); toast.error(result.error); } else { const count = result?.count ?? 1; const isMock = result?.mock; toast.success(isMock ? `تم توليد ${count} صورة (تجريبي)` : `تم توليد ${count} صورة`); setPrompt(""); router.refresh(); } }); }
  function handleDelete(itemId: string) { startTransition(async () => { const result: any = await deleteMoodboardItemAction(projectId, itemId); if (result?.error) toast.error(result.error); else { toast.success("تم الحذف"); router.refresh(); } }); }

...
export default async function TypographyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const data = typo?.data as Record<string, unknown> | undefined;
  const mockMode = getMockMode();
  const getFont = (key: string) => (((data?.[key] as Record<string, unknown> | undefined)?.family) as string | undefined);
  const previewTexts = data?.previewTexts as Record<string, string> | undefined;
  return (
    <div className="mx-auto max-w-5xl px-4 sm:px-6 py-8 sm:py-12">
      ...
      {typo && data ? (
        <div className="space-y-6">
          <section className="card-premium p-6"><h2 className="text-lg font-semibold mb-4">عائلات الخطوط</h2><div className="grid gap-4 sm:grid-cols-2"><FontCard label="عربي — عنوان رئيسي" font={getFont("arabicDisplayFont")} /><FontCard label="عربي — عناوين" font={getFont("arabicHeadingFont")} /><FontCard label="عربي — نصوص" font={getFont("arabicBodyFont")} /><FontCard label="إنجليزي — عنوان رئيسي" font={getFont("englishDisplayFont")} /><FontCard label="إنجليزي — عناوين" font={getFont("englishHeadingFont")} /><FontCard label="إنجليزي — نصوص" font={getFont("englishBodyFont")} /></div></section>
          {previewTexts && <section className="card-premium p-6"><h2 className="text-lg font-semibold mb-4">معاينات حية</h2><div className="space-y-4"><PreviewBlock label="العنوان الرئيسي" text={previewTexts.arabic} style={typo.styles.find((s) => s.name === "DISPLAY")} /><PreviewBlock label="النص" text={previewTexts.arabicBody} style={typo.styles.find((s) => s.name === "BODY")} /></div></section>}

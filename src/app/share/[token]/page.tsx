...
export default async function ShareTokenPage({ params, searchParams }: { params: Promise<{ token: string }>; searchParams: Promise<{ pw?: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 py-8 sm:py-12">
      ...
      <ClientPortal token={token} projectName={project.nameAr} allowComment={link.allowComment} allowApprove={link.allowApprove} approvalStatus={approved ? "APPROVED" : "PENDING"} comments={comments.map((c) => ({ id: c.id, authorName: c.authorName, authorType: c.authorType, body: c.body, createdAt: c.createdAt, status: c.status }))} />
      <div className="mt-12 space-y-6">
        ...
        {typography && typography.data && <section className="card-premium p-6"><div className="flex items-center gap-2 mb-4"><Type className="size-4 text-violet" /><h2 className="text-lg font-semibold">نظام الخطوط</h2></div><div className="space-y-2 text-sm"><div><span className="text-muted-foreground">عربي:</span> <span className="font-medium">{(typography.data as any).arabicBodyFont?.family ?? "—"}</span></div></div></section>}
      </div>
